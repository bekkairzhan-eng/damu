...
const BOTTOM_NAV = [
  { path: '/dashboard/my-dashboard', label: 'Главная',   icon: 'home'         },
  { path: '/plans',                  label: 'Развитие',  icon: 'flag'         },
  { path: '/career-map',             label: 'Карьера',   icon: 'account_tree' },
  { path: '/titles',                 label: 'Должности', icon: 'work'         },
  { path: '/skills',                 label: 'Навыки',    icon: 'school'       },
]
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
      ...
      <div style={{ display: 'flex', flex: 1, position: 'relative' }}>
        ...
        {/* Кнопка свернуть/развернуть — скрыта на мобилке */}
        {!isMobile && (
          <div style={{
            position: 'fixed',
            left: W,
        ...
        {/* Основной контент */}
        <div style={{ flex: 1, minWidth: 0, background: '#f5f6fa', paddingBottom: isMobile ? 64 : 0 }}>
          <Outlet />
        </div>

      </div>

      {/* Нижняя навигация — только мобилка */}
      {isMobile && (
        <nav style={{
          ...
          {BOTTOM_NAV.map(item => (
            <NavLink
              key={item.path}
              to={item.path}
              end={item.path === '/dashboard/my-dashboard'}
              style={({ isActive }) => ({
                flex: 1, display: 'flex', flexDirection: 'column',
                alignItems: 'center', justifyContent: 'center', gap: 2,
                color: isActive ? '#4361ee' : '#9aafbd',
