...
> **Открыто:** контракт интеграции с BILIM (событие о завершении курса/теста → авто-подтверждение уровня 1–2) и с TestLab (результат тестирования → авто-подтверждение уровня 3) не спроектирован — см. [HRD_QUESTIONS.md](HRD_QUESTIONS.md#навыки-и-подтверждение), вопрос №2.
...
> Все эндпоинты требуют роль `hr`. HR видит только сотрудников своего скоупа — см. [HRD_QUESTIONS.md](HRD_QUESTIONS.md#доступы-и-роли), вопрос №17. Эндпоинты по заявкам на обучение и сертификатам — см. раздел 5.7 выше.
...
> Снятие `on_bench` формально не определено бизнесом — предполагается, что произойдёт само при следующей синхронизации с HRMS, когда `position_id` сотрудника реально сменится (см. [HRD_QUESTIONS.md](HRD_QUESTIONS.md#аттестация), вопрос №12).
...
⚠️ Формат запроса к LMS — открытый вопрос, см. [HRD_QUESTIONS.md](HRD_QUESTIONS.md#интеграции-и-технические-контракты), вопрос №23. Зафиксировать с командой BPM/LMS (см. [BPM_INTEGRATION.md](BPM_INTEGRATION.md#3-заявка-на-обучение)).
