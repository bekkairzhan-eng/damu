...
> **Решено 31.07.2026:** подтверждение навыка вне карьерного трека идёт по тому же пайплайну (BILIM/TestLab/эксперт), без исключений — см. открытый вопрос №20 в общем списке.
...
> **Открыто:** контракт интеграции с BILIM (событие о завершении курса/теста → авто-подтверждение уровня 1–2) и с TestLab (результат тестирования → авто-подтверждение уровня 3) не спроектирован — уточнить формат события/API с этими командами.
...
> **Открыто (встреча 24.07.2026, см. открытый вопрос №18):** добавляется 6-й компонент «Оценка эффективности 9 клеток» с **динамическим** весом (зависит от клетки сотрудника), а не фиксированным как остальные 5 — текущая модель `component enum PK / weight smallint, сумма=100` рассчитана только на фиксированные веса. Требует пересмотра перед реализацией.
...
Сотрудник выставляет уровень навыка. Навык может быть любым из каталога `skills`, не только целевым (не входящим в `requirements` его должности) — см. открытый вопрос в разделе 4 про подтверждение таких навыков.
