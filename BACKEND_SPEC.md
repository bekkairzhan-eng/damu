...
| LMS | BILIM | Курсы/сертификаты + (с 23.07.2026) подтверждение навыков уровня 1–2 |
| Тестирование | **TestLab** (новая интеграция) | Подтверждение навыков уровня 3 по результатам тестирования — контракт не определён, см. открытые вопросы |
| Фронтенд | React 19 + Vite | Прототип готов |
| Деплой | Kubernetes | Все окружения (dev / staging / prod) |
| API | REST (JSON) | — |

> **Изменение по итогам встречи с заказчиком 23.07.2026:** сотрудник больше не подтверждает навыки самостоятельно ни на одном уровне. Уровни 1–2 — через BILIM, 3 — через TestLab, 4 — вручную через UnityBPM (единственный уровень с ручным апрувом). Подробности — [BPM_INTEGRATION.md, раздел 1](BPM_INTEGRATION.md#1-подтверждение-навыка).
...
| `grade` | smallint | **Из HRMS через Kafka. В Damu только чтение** — админ не редактирует (решено 12.08.2026, см. ниже) |
| `track` | varchar | Полевой состав / Инженерный трек / Управление объектом / Управление проектом |
| `description` | jsonb | Краткое описание должности (1–2 фразы). Заводит админ |
| `kb_url` | varchar | Ссылка «Читать в Базе Знаний» — ведёт в **пространство Damu** в Базе знаний. Заводит админ per-position |
| `salary_min` | numeric | Из HRMS, только чтение. Показывать ли сотрудникам — не решено, см. [HRD_QUESTIONS.md](HRD_QUESTIONS.md#должности), вопрос №11 |
...
MVP-должности: Foreman D/C/B/A, Site Engineer D/C/B/A, Site Manager, Deputy PM, Project Manager.
...
> **Грейды — источник правды HRMS (решено 12.08.2026).** Приходят через Kafka. Должности уже синхронизируются, грейды — следующий шаг: связать их с `positions.grade`. `PUT /api/admin/positions/:id` **не должен** принимать `grade` — попытка изменить возвращает 422. До этого решения грейды в прототипе расходились между экранами (на «Должностях» Foreman C был 13, в админке — 14).

---

### `req_categories`

> **Новое, решено 12.08.2026.** Категории «Общих требований» — **общий справочник для всех должностей** (а не свой набор у каждой). Редактируется админом на `/admin/positions`.

| Поле | Тип | Описание |
|------|-----|---------|
| `id` | uuid PK | — |
| `name` | jsonb | Например «Работа с заказчиком» |
| `icon` | varchar | Имя иконки material-symbols |
| `sort_order` | smallint | Порядок вывода в таблице на `/titles` |

Начальный набор (6): Работа с заказчиком, Управление командой, Строительные практики, Ответственность и полномочия, Отчётность и документация, Опыт и требования.

**Удаление категории** каскадно удаляет строки `position_general_requirements` по ней у всех должностей — в UI об этом предупреждает диалог.

---

### `position_general_requirements`

Текст «Общих требований»: `должность × категория → текст`. Заводит админ в `/admin/positions/:id`.

| Поле | Тип | Описание |
|------|-----|---------|
| `position_id` | uuid FK → positions | — |
| `req_category_id` | uuid FK → req_categories | — |
| `text` | jsonb | **Обязателен, не может быть пустым** |

PK: `(position_id, req_category_id)`.

> **Правило обязательности (решено 12.08.2026).** Категория — общая, поэтому добавление новой категории делает её обязательной к заполнению у **каждой** должности. Чтобы не возникала «шахматка» из пропусков:
> - `PUT /api/admin/positions/:id/content` возвращает **422**, если хотя бы по одной категории из `req_categories` текст пустой или отсутствует — проверка на бэке, не только на фронте;
> - `GET /api/admin/positions` отдаёт по каждой должности `reqs_filled` / `reqs_total`, чтобы админка показывала счётчик «5/7» и баннер со списком незаполненных должностей;
> - на `/titles` незаполненная ячейка отображается как «Не заполнено» — молча пустую ячейку не показываем.
...
**Счётчик «+N добавили» в каталоге (решено 31.07.2026):** `added_count` = количество сотрудников, у которых в `employee_skills` есть строка с этим `skill_id` **добровольно** — т.е. `is_favorite = true` ИЛИ навык не является целевым для их должности (не найден в `requirements` для их position/cluster). Строки, появившиеся только потому, что навык целевой (обязательное требование должности), в счётчик **не входят** — иначе популярными будут любые навыки, обязательные для распространённых должностей, что не отражает реальный интерес. Пересчитывать вместе с рейтинговым батчем или отдельным заданием — не критично к реальному времени.

---

### `skill_levels`

> **Новое, решено 07.08.2026.** Карта развития компетенций и описание уровней (пробелы из шаблона Role Profile HRD) — контент теперь на уровне справочника навыка, а не только внутри плана конкретного сотрудника. Заполняется админом через `/admin/skills/:id`.

| Поле | Тип | Описание |
|------|-----|---------|
| `skill_id` | uuid FK → skills | — |
| `level` | smallint | 1–4 |
| `description` | jsonb | Что нужно знать/уметь на этом уровне |
| `bilim_course_url` | varchar? | Только для уровней 1–2. Ссылка на курс в BILIM |
| `testlab_test_id` | uuid FK → testlab_tests? | Только для уровня 3. Привязка к конкретному тесту (не произвольная ссылка) |

PK: `(skill_id, level)`. Для уровня 4 `bilim_course_url`/`testlab_test_id` не используются — подтверждение полностью ручное (эксперт/руководитель через BPM).
...
|------|-----|---------|
| `skill_id` | uuid FK → skills | — |
| `type` | varchar | `knowledge_base` \| `mandatory_path` |
| `title` | jsonb | — |
| `url` | varchar | — |
...
> Справочник тестов TestLab. Пока ведётся вручную админом в Damu (`data/testlabTests.js` в прототипе) — контракт синхронизации из самого TestLab не определён, см. [HRD_QUESTIONS.md](HRD_QUESTIONS.md#навыки-и-подтверждение), вопрос №2.

| Поле | Тип | Описание |
|------|-----|---------|
| `id` | uuid PK | — |
| `name` | jsonb | Название теста |
| `created_at` | timestamptz | — |
| `created_by` | uuid FK → employees | Кто добавил (Админ) |

Используется через `skill_levels.testlab_test_id` (только для уровня 3). Проверка результата — не webhook в реальном времени, а отдельный периодический процесс **раз в полгода** (не тот же батч, что ночной пересчёт `profile_rating`, — расписание/джоба для него пока не спроектированы): система смотрит, прошёл ли сотрудник привязанный к его навыку тест, и если да — выставляет `employee_skills.status = confirmed`, `confirmed_via = testlab` для уровня 3.
...
> **Изменено 23.07.2026:** `status`/`confirmed_by` больше не означают только «сам / через BPM» — подтверждение теперь идёт разными источниками в зависимости от уровня (см. `confirmed_via`).
> **Дополнено 31.07.2026** (обнаружен пробел в ТЗ при ревью): раздел «Мои навыки» во фронтенде группирует навыки на **Все / Избранные / Подтверждённые / Целевые** — этой логики не было ни в модели данных, ни в API. Добавлены `is_favorite` и понятие «целевой навык» (см. ниже).
> **Уточнено 31.07.2026:** `current_level` — **не `null`, а `0`** («не начат») по умолчанию, для единообразия с `requirements.required_level` (там тоже 0 = не требуется). Подтверждено заказчиком: нельзя запросить подтверждение уровня 4 (Эксперт), не имея подтверждённого уровня 3 (Продвинутый) — это уже было заложено в логику `confirmed_via`/уровневом флоу, здесь только поправлено представление «нет уровня».
...
| `skill_id` | uuid FK → skills | — |
| `current_level` | smallint | 0–4, по умолчанию **0** («не начат» — навык добавлен, но уровень ещё не пришёл ни от BILIM, ни от TestLab). 1–4 — реальный уровень |
| `status` | enum? | `pending` / `confirmed` / `rejected` — ~~`self`~~ **больше не используется**, сотрудник не подтверждает сам ни один уровень. `null`, пока `current_level = 0` |
| `confirmed_via` | enum? | `bilim` (уровни 1–2) / `testlab` (уровень 3) / `expert` (уровень 4, через BPM) — источник подтверждения |
| `confirmed_by` | varchar | Имя апрувера — заполняется только когда `confirmed_via = expert` (из BPM-webhook). Для `bilim`/`testlab` — не заполняется (это система, не человек) |
| `confirmed_at` | timestamptz | — |
| `is_favorite` | boolean | Свободно переключается сотрудником **только среди навыков, уже присутствующих в его профиле** (`/dashboard/my-skills`) — не из общего каталога `/skills` (решено 31.07.2026, см. ниже) |
| `updated_at` | timestamptz | — |

Уникальный: `(employee_id, skill_id)`.

**Важно: `employee_skills` не ограничена навыками из `requirements`.** Сотрудник может добавить в «Мои навыки» любой навык из общего каталога `skills` через раздел «Навыки» (`/skills`) — даже если этот навык не требуется ни на его текущей, ни на целевой должности. Строка `employee_skills` создаётся, когда сотрудник добавляет навык «в план» из каталога — `current_level = 0`, `status`/`confirmed_via` — `null`.

**Каталог (`/skills`) — только добавление/удаление, без избранного (решено 31.07.2026).** Раздел «Навыки» — это общий каталог компании, в нём нет ни фильтра «Избранные», ни возможности отметить навык избранным напрямую. Единственное действие там — добавить навык в профиль («В план») или убрать его обратно (см. ниже про удаление). Избранное — только внутри «Мои навыки», как способ отфильтровать уже присутствующие в профиле навыки; это не способ добавить туда что-то новое.

**Удаление навыка из «Мои навыки» через каталог (решено 31.07.2026).** Как и на самом экране «Мои навыки», убрать навык можно только если `current_level = 0` и навык не целевой (не найден в `requirements` для должности сотрудника) — доступно и на экране «Мои навыки», и прямо в каталоге/на деталке навыка (везде, где навык уже показан как «в профиле»). Как только уровень становится ≥1 — удаление недоступно нигде.

**«Целевой навык» — не хранимое поле, а вычисляемое:**
```
is_target = EXISTS (
  SELECT 1 FROM requirements r
  WHERE r.skill_id = employee_skills.skill_id
    AND r.cluster_id = employee.cluster_id
    AND r.required_level > 0
    AND r.position_id IN (employee.position_id, active_career_plan.target_position_id)
)
```
То есть навык считается целевым, если он требуется на текущей ИЛИ на целевой должности сотрудника в его кластере. Это то же самое соединение, что уже используется для расчёта `skillGroups`/`gap` в карьерном плане (`career_plans` → `requirements`) — новой сущности не требуется, только явно задокументировать.

**Семантика 4 групп на экране «Мои навыки»:**

| Группа | Что показывает |
|--------|-----------------|
| **Целевые** | `is_target = true` — включает навыки без `employee_skills`-записи вовсе (ещё не начатые, `gap`, level 0) и с записью (в процессе/подтверждён) |
| **Подтверждённые** | `employee_skills.status = 'confirmed'`, независимо от `is_target` |
| **Избранные** | `employee_skills.is_favorite = true`, независимо от `is_target`/`status` |
| **Все** | Объединение: все навыки с `is_target = true` (включая ещё не начатые) **∪** все существующие строки `employee_skills` сотрудника (в т.ч. навыки вне карьерного трека, добавленные самостоятельно) |

> **Решено 31.07.2026:** подтверждение навыка вне карьерного трека идёт по тому же пайплайну (BILIM/TestLab/эксперт), без исключений — см. [HRD_QUESTIONS.md](HRD_QUESTIONS.md#закрытые-вопросы-лог-решений).

**Исключение навыка из профиля (новое, решено 31.07.2026):** сотрудник может убрать самостоятельно добавленный навык из «Мои навыки» **только пока `current_level IS NULL`** — то есть навык был добавлен ("В план"/избранное), но уровень ещё ни разу не выставлялся. Как только выставлен любой уровень (даже 1 — Базовый), запись становится постоянной и исключить её нельзя — ни через UI, ни через API (проверка на бэке, не только на фронте). Это осознанное упрощение: раз уровень ещё не выставлен, запрос на подтверждение (BILIM/TestLab/эксперту) никогда не отправлялся, поэтому удаление не требует отмены/отзыва внешнего процесса — в отличие от `training_requests`, где предусмотрен статус `withdrawn`.

- Целевой навык (`is_target = true`) исключить нельзя никогда — это вычисляемое свойство от `requirements`, а не хранимая запись; "исключение" здесь не имеет смысла, поскольку требование должности всё равно останется видно как `gap`.
- `DELETE /api/me/skills/:skillId` (см. раздел 5.1) — возвращает 422, если `current_level != 0`.

> **Открыто:** контракт интеграции с BILIM (событие о завершении курса/теста → авто-подтверждение уровня 1–2) не спроектирован. Для уровня 3 механика на стороне Damu решена (`skill_levels.testlab_test_id` + проверка раз в полгода в рейтинговом батче, см. `testlab_tests` выше), но сам контракт с TestLab (как получить и как трактовать результат конкретного теста) — всё ещё не определён — см. [HRD_QUESTIONS.md](HRD_QUESTIONS.md#навыки-и-подтверждение), вопрос №2.

---

### `skill_approval_history`

История всех попыток подтверждения навыка. Хранится в Damu. Актуальна в полном виде (`bpm_task_id`, `approver_name`) только для уровня 4 — для уровней 1–3 запись тоже создаётся (для единообразной истории), но `approver_name` = название системы (`'BILIM'`/`'TestLab'`), `bpm_task_id` — пусто.
...
| `bpm_task_id` | varchar? | ID задачи в UnityBPM — только для уровня 4 |
| `decision` | enum | `approved` / `rejected` |
| `approver_name` | varchar | Для уровня 4 — имя апрувера из BPM-webhook; для уровней 1–3 — `'BILIM'` / `'TestLab'` |
| `comment` | text | Комментарий апрувера. **Обязателен при любом `decision`** для уровня 4 (и `approved`, и `rejected`) — по итогам встречи 23.07.2026 требование расширено с «только при отклонении». Валидировать на входе вебхука, отклонять payload без комментария (422) |
...
Проекты, добавленные сотрудником вручную или полученные из SuperApp. Основной проект (1 шт., только чтение) — в `employees.project_hrms`.

> **Новое 23.07.2026:** проекты, введённые сотрудником вручную, теперь требуют подтверждения HR через BPM (по каждому проекту отдельно) — см. [BPM_INTEGRATION.md, раздел 4](BPM_INTEGRATION.md#4-подтверждение-проекта-опыта-работы). Проекты из SuperApp (когда интеграция готова) считаются доверенными и создаются сразу подтверждёнными, без BPM.
...
| `description` | text | — |
| `source` | enum | `manual` (сотрудник ввёл сам) / `superapp` (новое, авто из SuperApp) |
| `status` | enum | `pending` (только для `source=manual`, ждёт HR) / `confirmed` / `rejected`. Для `source=superapp` — всегда `confirmed` сразу при создании |
| `confirmed_by` | varchar? | Имя HR, подтвердившего проект (из BPM-webhook). Пусто для `source=superapp` |
| `rejection_comment` | text? | Комментарий HR при отклонении — обязательность и точный формат не решены, см. открытые вопросы в BPM_INTEGRATION.md |
| `created_at` | timestamptz | — |

> **Открыто:** нужен новый вебхук `POST /webhooks/bpm/project-approval` (по аналогии с `skill-approval`) — контракт не спроектирован. Формат интеграции с SuperApp (push события о новых проектах? периодическая синхронизация?) тоже не определён — SuperApp пока не готов как продукт.
...
> **Открыто (встреча 24.07.2026):** добавляется 6-й компонент «Оценка эффективности 9 клеток» с **динамическим** весом (зависит от клетки сотрудника), а не фиксированным как остальные 5 — текущая модель `component enum PK / weight smallint, сумма=100` рассчитана только на фиксированные веса. Требует пересмотра перед реализацией. См. [HRD_QUESTIONS.md](HRD_QUESTIONS.md#рейтинг-профиля), вопрос №14.
...
#### `GET /api/me/skills?group=all|target|confirmed|favorite`

Навыки сотрудника с текущим уровнем, статусом подтверждения, признаками целевого/избранного навыка. Соответствует 4 группам-фильтрам на экране «Мои навыки» (см. `employee_skills` в разделе 4 — там же описана формула `is_target` и семантика групп).

`group` — необязательный query-параметр:
- `target` — только навыки с `isTarget = true` (включает навыки без записи `employee_skills`, т.е. ещё не начатые — `currentLevel: null`, `status: null`)
- `confirmed` — только `status = confirmed`
- `favorite` — только `isFavorite = true`
- без параметра или `all` — объединение: все целевые навыки (включая не начатые) + все навыки, у сотрудника есть запись `employee_skills` (в т.ч. вне карьерного трека)
...
> **Обновлено 31.07.2026** — этот эндпоинт описывал устаревшую логику самоподтверждения уровня 1–2, отменённую ещё 23.07.2026 (см. `employee_skills`); при пересмотре модели данных обновить сам эндпоинт забыли. Актуальная логика ниже.

Сотрудник выставляет уровень навыка. Навык может быть любым из каталога `skills`, не только целевым (не входящим в `requirements` его должности) — подтверждение таких навыков решено 31.07.2026, см. [HRD_QUESTIONS.md](HRD_QUESTIONS.md#закрытые-вопросы-лог-решений).
...
**Логика:**
- Уровень 1–2: записывает `status = pending`, `confirmed_via = bilim` — ждём события от BILIM (курс/тест), апрув сотрудника не требуется и невозможен
- Уровень 3: записывает `status = pending`, `confirmed_via = testlab` — ждём результата тестирования
- Уровень 4: записывает `status = pending`, `confirmed_via = expert`, отправляет запрос в UnityBPM эксперту/руководителю (см. раздел интеграций), создаёт запись в `skill_approval_history`
- Ни на одном уровне сотрудник не подтверждает навык сам — во всех случаях запись сразу уходит в `pending` до ответа BILIM/TestLab/эксперта
...
```json
{
  ...
  "currentLevel": 3,
  "status": "pending",
  "confirmedVia": "testlab"
}
```

---

#### `PATCH /api/me/skills/:skillId/favorite`

**Новое.** Переключает избранное — не зависит от уровня/статуса подтверждения. **Вызывается только с экрана «Мои навыки»** (`/dashboard/my-skills`) — раздел «Навыки» (`/skills`) избранное не показывает и не переключает (решено 31.07.2026, см. `employee_skills`). Строка `employee_skills` для навыка уже существует к этому моменту (он не может быть в «Мои навыки», если строки нет) — эндпоинт не создаёт новых строк.
...
**Новое, решено 31.07.2026.** Убирает навык из профиля сотрудника — только если уровень ещё не выставлен. Вызывается как с «Мои навыки», так и из каталога `/skills`/деталки навыка (везде, где навык уже показан как «в профиле») — логика одна и та же.

**Логика:**
- `current_level = 0` → удаляет строку `employee_skills`, ответ `204 No Content`
- `current_level != 0` (выставлен любой уровень, хоть Базовый) → **422**, тело `{ "error": "level_already_set" }` — навык больше нельзя убрать, независимо от того, подтверждён он, отклонён или ещё ожидает
- Целевые навыки (`isTarget = true`) — эндпоинт неприменим никогда, даже если строки `employee_skills` ещё нет (чистый `gap`) или уже есть с `current_level = 0`: требование всё равно вычисляется из `requirements` и никуда не денется
...
> Все эндпоинты требуют роль `hr`. HR видит только сотрудников своего скоупа — см. [HRD_QUESTIONS.md](HRD_QUESTIONS.md#доступы-и-роли), вопрос №16. Эндпоинты по заявкам на обучение и сертификатам — см. раздел 5.7 выше.

#### `GET /api/hr/employees`

Справочник сотрудников в скоупе HR — для экрана «Сотрудники» и для поиска сотрудника при ручном добавлении сертификата.

**Query params:** `?search=&clusterId=uuid&page=1&limit=20`
...
```json
{
  "total": 128,
  "items": [
    { "id": "uuid", "fullName": "Данияр Сейтжанов", "position": "Foreman B", "organization": "BI Construction", "cluster": { "id": "uuid", "code": "K2" } }
  ]
}
```
...
> Снятие `on_bench` формально не определено бизнесом — предполагается, что произойдёт само при следующей синхронизации с HRMS, когда `position_id` сотрудника реально сменится (см. [HRD_QUESTIONS.md](HRD_QUESTIONS.md#аттестация), вопрос №10).
...
`PUT /api/admin/positions/:id` — обновить описание, `kb_url`, `track`. **`grade` не принимается** — источник правды HRMS (Kafka), попытка изменить → 422. Зарплатные вилки — тоже только из HRMS.

`PUT /api/admin/positions/:id/content` — «Общие требования» по категориям + описание + `kb_url`. Тело: `{ description, kbUrl, generalReqs: { <reqCategoryId>: "текст" } }`. Возвращает **422**, если по любой категории из `req_categories` текст пустой — все категории обязательны (см. `position_general_requirements`).

`GET/POST/PUT/DELETE /api/admin/req-categories` — общий справочник категорий «Общих требований». `DELETE` каскадно удаляет тексты по этой категории у всех должностей — в ответе вернуть количество затронутых должностей, чтобы UI показал подтверждение.
...
| `hrms.position.updated` | Потребитель | HRMS публикует должности и **грейды** (решено 12.08.2026 — грейд источник правды HRMS, в Damu только чтение). Должности уже тянутся, грейды — следующий шаг: связать с `positions.grade` |
...
⚠️ Формат запроса к LMS — открытый вопрос, см. [HRD_QUESTIONS.md](HRD_QUESTIONS.md#интеграции-и-технические-контракты), вопрос №22. Зафиксировать с командой BPM/LMS (см. [BPM_INTEGRATION.md](BPM_INTEGRATION.md#3-заявка-на-обучение)).
...
| 9 | Карта развития компетенций (по письму HRD, шаблон Role Profile): нужна новая сущность на уровне навыка — способы развития (обучение/наставничество/проект/сертификация), не только `plan_learning_items` внутри плана сотрудника. Проектировать сейчас или после сбора данных по шаблону (дедлайн 15 августа)? |
| 10 | Описание уровней компетенций: завести `skill_level_descriptions` (skill_id × уровень 1–4 × текст) вместо текущего одинакового для всех навыков текста в `SkillDetail.jsx` — данные придут из шаблонов HRD |
| 11 | Кадровый резерв, преемники, горизонтальные карьерные переходы между функциями — упомянуты в письме HRD как ожидаемая функциональность BI DAMU, но не описаны нигде в текущей модели. В скоупе MVP или следующая итерация? |
| 12 | Контракт с BILIM для подтверждения навыков уровня 1–2: что именно триггерит подтверждение (завершение курса? проходной балл теста внутри LMS?), формат события/API — не определено (встреча с заказчиком 23.07.2026) |
| 13 | Контракт с TestLab для подтверждения навыка уровня 3: формат результата тестирования, порог зачёта, API/событие — TestLab не описан нигде до этой встречи, интеграция с нуля |
| 14 | Роль «Руководитель»/«Эксперт» для апрува навыка уровня 4 — в Damu сейчас нет такой роли (только employee/hr/admin). Нужна оргструктура «кто чей руководитель» (из HRMS?) для определения апрувера |
| 15 | Вебхук/эндпоинт для подтверждения проекта (`project-approval`) — новая сущность в BPM (см. BPM_INTEGRATION.md, раздел 4), контракт не спроектирован |
| 16 | Формат интеграции с SuperApp для получения проектов сотрудника — SuperApp пока не готов как продукт, формат push/pull не обсуждался |
| 17 | Интеграция с HappyJob — нужна ли выгрузка данных (результаты опросов вовлечённости/eNPS?) в Damu, и куда именно в модели данных это должно попасть — зафиксировано только как идея, без деталей |
| 18 | 6-й компонент рейтинга «Оценка эффективности 9 клеток» (встреча 24.07.2026): вес компонента зависит от клетки (performance × potential), в которую попал сотрудник — динамический вес вместо фиксированного. Ломает текущую модель `rating_weights` (5 полей, фиксированный вес каждого, сумма = 100%) — нужно либо добавить 6-е поле с особой логикой расчёта веса, либо перепроектировать модель весов |
| 19 | «Корпоративная активность» теперь 5 показателей вместо 3 (добавлены «Грамоты/государственные награды» и «Нахождение в кадровом резерве», реализовано в прототипе 24.07.2026 — `CORP_ITEMS` в `ExperienceProfile.jsx`) — источник данных для двух новых пунктов не определён (HRMS? ручной ввод HR, как остальные три?) |
| 20 | ~~Нужно ли подтверждение (BILIM/TestLab/эксперт) для навыка, который сотрудник добавил сам и который не входит в требования его должности (не целевой)?~~ — **решено 31.07.2026:** да, тот же пайплайн без исключений. См. также правило исключения навыка из профиля в `employee_skills` (раздел 4) |
| 21 | Новый эндпоинт `DELETE /api/me/skills/:skillId` (исключение навыка без уровня из профиля) — контракт описан в разделе 4/5.1, реализация не начата |
