...
| `status` | enum | `pending` / `confirmed` / `rejected` — ~~`self`~~ **больше не используется**, сотрудник не подтверждает сам ни один уровень |
| `confirmed_via` | enum | `bilim` (уровни 1–2) / `testlab` (уровень 3) / `expert` (уровень 4, через BPM) — источник подтверждения. Новое поле, заменяет неявное предположение "self vs BPM" |
