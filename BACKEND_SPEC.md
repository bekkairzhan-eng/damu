...
| `skill_id` | uuid FK → skills | — |
| `current_level` | smallint? | 1–4 — уровень, выставленный сотрудником. **Может быть `null`** — см. ниже про навыки без уровня |
| `status` | enum? | `pending` / `confirmed` / `rejected` — ~~`self`~~ **больше не используется**, сотрудник не подтверждает сам ни один уровень. `null`, если уровень ещё не выставлен |
...
| `is_favorite` | boolean | **Новое.** Свободно переключается сотрудником, никак не связано с целевыми/подтверждёнными навыками — «может быть что угодно» (по формулировке заказчика) |
| `updated_at` | timestamptz | — |

Уникальный: `(employee_id, skill_id)`.

**Важно: `employee_skills` не ограничена навыками из `requirements`.** Сотрудник может добавить в «Мои навыки» любой навык из общего каталога `skills` через раздел «Навыки» (`/skills`) — даже если этот навык не требуется ни на его текущей, ни на целевой должности. Строка `employee_skills` создаётся при любом из двух действий: (а) сотрудник выставил уровень навыка, (б) сотрудник добавил навык в избранное (`is_favorite = true`) без уровня — тогда `current_level`/`status`/`confirmed_via` остаются `null`.
...
- `DELETE /api/me/skills/:skillId` (см. раздел 5.1) — возвращает 422, если `current_level IS NOT NULL`.
...
**Новое.** Переключает избранное — не зависит от уровня/статуса подтверждения. Если строки `employee_skills` для этого навыка ещё нет (сотрудник просто лайкнул навык в каталоге `/skills`, уровень не выставлял), создаёт её с `current_level: null`.
...
**Новое, решено 31.07.2026.** Исключает навык из «Мои навыки» — только если уровень ещё не выставлен.

**Логика:**
- `current_level IS NULL` → удаляет строку `employee_skills`, ответ `204 No Content`
- `current_level IS NOT NULL` (выставлен любой уровень, хоть Базовый) → **422**, тело `{ "error": "level_already_set" }` — навык больше нельзя исключить, независимо от того, подтверждён он, отклонён или ещё ожидает
- Целевые навыки (`isTarget = true` без записи `employee_skills`, т.е. чистый `gap`) — эндпоинт неприменим, удалять нечего, требование всё равно вычисляется из `requirements`
